<x-app-layout>
    <div class="container mx-auto px-4 py-6">
        <div class="max-w-4xl mx-auto">
            <div class="bg-white rounded-lg shadow p-6 mb-6">
                <h2 class="text-2xl font-bold text-gray-800">Editar Equipo: {{ $equipo->Nombre }}</h2>
            </div>
            
            <div class="bg-white rounded-lg shadow p-6">
                <form action="{{ route('otros-equipos.update', $equipo->id) }}" method="POST">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       id="Num_inv" 
                                       name="Num_inv" 
                                       value="{{ old('Num_inv', $equipo->Num_inv) }}"
                            ...
                            <div>
                                ...
                                <select id="estado_id" 
                                    ...
                                    @foreach($estados as $estado)
                                        <option value="{{ $estado->id }}" {{ old('estado_id', $equipo->estado_id) == $estado->id ? 'selected' : '' }}>
                                            {{ $estado->nombre }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input type="text" 
                                       id="Asignacion" 
                                       name="Asignacion" 
                                       value="{{ old('Asignacion', $equipo->Asignacion) }}"
                                       class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                                @error('Asignacion')
                                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="mb-8">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <select id="zona_id" 
                                        name="zona_id" 
                                        {{ !$equipo->edificio_id && !old('edificio_id') ? 'disabled' : '' }}
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                            ...
                            <div>
                                ...
                                <select id="cubiculo_id" 
                                        name="cubiculo_id" 
                                        {{ !$equipo->zona_id && !old('zona_id') ? 'disabled' : '' }}
                                        class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div class="mb-8">
                        ...
                        <textarea id="observaciones" 
                                  name="observaciones" 
                                  rows="3"
                                  class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent">{{ old('observaciones', $equipo->observaciones) }}</textarea>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
            ...
            if (edificioId) {
                fetch(`/zonas?edificio_id=${edificioId}`)
                    ...
                    .then(data => {
                        ...
                        @elseif($equipo->zona_id)
                            zonaSelect.value = '{{ $equipo->zona_id }}';
                            zonaSelect.dispatchEvent(new Event('change'));
                        @endif
                    })
                    .catch(error => {
            ...
            if (zonaId) {
                fetch(`/cubiculos?zona_id=${zonaId}`)
                    ...
                    .then(data => {
                        ...
                        @elseif($equipo->cubiculo_id)
                            cubiculoSelect.value = '{{ $equipo->cubiculo_id }}';
                        @endif
                    })
                    .catch(error => {
                        console.error('Error:', error);
        ...
        @if(old('edificio_id') || $equipo->edificio_id)
            // Disparar el evento change para cargar las zonas
            edificioSelect.value = '{{ old('edificio_id', $equipo->edificio_id) }}';
